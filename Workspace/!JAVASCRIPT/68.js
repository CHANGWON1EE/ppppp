...
// 가상 선택자 사용 불가 => ex) nth-of-type, nth-of-child
...
형식    :   요소.style.value = 문자열 형식의 value

            -  input 태그와 같이 닫는 태그가 없어 value 속성 지정이 가능한
               태그에 대한 값 반환 및 지정.

형식    :   요소.style.textContent = 문자열 형식의 value
...
if (inputTag[0].value == '다음') {   // value => 닫는 특성을 가지지 않은 태그에 사용
    inputTag[0].value = 'Next';      // 태그 대부분은 닫는 태그가 있지만 input tag는 닫는 태그가 없음
}

console.log(menu[1].textContent);
if (menu[1].textContent == '카페') { // text content => 안에 있는 콘텐트만 가져옴
    menu[1].textContent = 'Cafe';    // tag 수정 불가
}

console.log(li[1].innerHTML);        // innerHTML => tag 수정을 원할 경우 사용
                                     // 콘텐트 뿐만 아니라 태그까지 다 가져옴
li[1].innerHTML = '<h3><a href="a" class="menu cafe">다락방</a></h3>' 
