...
console.log(func2);                 // func2에 대한 var 선언으로 변수 자체는 호이스팅되지만
                                    // 아래 정의한 함수 표현식은 호이스팅되지 않아 undefined 반환
...
console.log(func2);         // 값부의 주소를 운용 객체 형태로 / 참조로 받는다
console.log(func2(1, 100)); // 함수 표현식 => 호이스팅 가능

// refVar();                // 호이스팅은 함수 호출에 대한 호이스팅일뿐 함수 내에서의 let구문
let value = '값';           // 외부 전역변수에 대한 참조가 호이스팅되지는 않음에 따라 컴파일 에러
                            // var구문으로 변수을 선언한 경우에도 변수
