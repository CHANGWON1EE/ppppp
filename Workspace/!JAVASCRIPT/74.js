...
/* blackBgChangeText_Btn.addEventListener('click', () => { // 똑같은걸 추가하기 때문 충돌이 일어남
    console.log(blackBgChangeText_Btn.value);
...
blackBgChangeText_Btn.addEventListener('click', () => { // 람다식에서는 this 활용 불가능
    console.log(this.value);
    if(this.value === '블랙배경') {                       // this.value = undefiend.value
        this.value = '화이트배경';
        changeBlackBg();
        body.style.color = 'white';
    } else {
        this.value = '블랙배경';                          // this.value = undefined.value
        changeWhiteBg();
        body.style.color = 'black';
    }
})
...
/* blackBgChangeText_Btn.addEventListener('click', function() {
    console.log(this.value);
    if(this.value === '블랙배경') { // blackBgChangeText_Btn.value
        this.value = '화이트배경';
        changeBlackBg();
        body.style.color = 'white';
    } else {
...
commit_Btn.addEventListener('click', () => {
    h1.textContent = h1.textContent.concat(' 완료');
    h1.style.color = 'blue';
})
