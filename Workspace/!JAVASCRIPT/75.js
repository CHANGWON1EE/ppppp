...
// 익명함수에서의 this => 자기자신 | 람다식에서의 this => undefined
...
btn[0].addEventListener('dblclick', () => {    // btn[0]을 더블클릭하면
    btn[1].style.cursor = 'none';              // *btn[1]의 커서 모양만 'none' 상태로 변경
});                                            // 눈에 보이는 상태만 'none'일뿐 기능(동작)은 정상적으로 작동
...
btn[1].addEventListener('dblclick', event => { // event => 매개변수(변수명은 중요치 않음)
    event.target.style.cursor = 'wait';        // event.target => this와 같음 | wait => 모래시계 or 딜레이 모양 출력
});

btn[2].addEventListener('dblclick', event => {
    event.target.style.cursor = 'pointer';     // event.target => this와 같음 | pointer => 손가락 모양
});
...
btn[3].addEventListener('dblclick', () => {    // 
    btn[0].style.pointerEvents = 'none';       // *btn[0]의 기능(동작) 자체를 'none' 상태로 변경 = 작동 중단
});                                            // style.cursor와 style.pointerEvents 확실한 구분 필요

btn[4].addEventListener('dblclick', () => {
    span[0].style.cursor = 'default';          // span[0]의 커서 모양을 브라우저별 기본 모양으로 변경
});

/* ==================================================================================================== */

span[0].addEventListener('dblclick', () => {
    btn[0].style.pointerEvents = 'auto';       // *btn[0]의 기능(동작) 자체를 'auto' 상태로 변경 => 작동 재개
});

/* ==================================================================================================== */

// < 2. mousedown, mouseup >
// 마우스 클릭 한 번을 down, up 두 동작으로 나누어 설정
/* 
    < mousedown >
...
box1[0].addEventListener('mousedown', function () { // mouse 클릭(down)했을 때 글자 크기 2em으로 변경
    this.style.fontSize = '2em';
});

box1[0].addEventListener('mouseup', function () {   // mouse 클릭 해제(up)했을 때 글자 크기 
...
box3[0].addEventListener('mousemove', function (event) {
    console.log('pageX ' + event.pageX);        // 전체페이지를 기준으로 동작 => 전체 문서에서의 좌표
    console.log(event.pageY);
    // console.log('offsetX ' + event.offsetX); // 부모요소를 기준으로 동작
