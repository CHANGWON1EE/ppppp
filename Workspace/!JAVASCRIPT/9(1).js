...
while (1) {        // C언어와 같이 0으로 평가되는 falsy값이 아닌 모든 값을 truthy값으로 평가.
    ...
    //                       //              비교시 숫자 타입으로 형변환되어 비교. 따라서 입력된 문자열 '0'과 '확인' 또는
    sum = sum + value;            //              '취소' 버튼을 눌렀을 때 반환되는 빈 문자열  ('')과 null은 모두 숫자 0으로
    //                       //              변환되어 비교. 단, 숫자 형태의 문자열과 숫자 형태가 아닌 모든 문자열은
    //                       //              복합 대입 연산자에 의해 문자열 결합 연산 처리되는 것을 확인 가능.
}
alert('합계: ' + sum);
 */
...
while (true) {
  ...
  if (!value) break;      // 입력된 값이 0이나 입력없이 '확인' 또는 '취소' 버튼을 눌렀을 때 반환되는 빈 문자열
