...
function test1(n1, n2, n3 = 5) { // Default parameter(디폴트 파라미터)
    return n1 + n2 + n3;
}
console.log(test1(1, 2, 3));     // => 1 + 2 + 3
console.log(test1(1, 2));        // => 1 + 2 + 5 | 실인수가 형식인수에 모두 대응되면 디폴트 파라미터의 초기식은 무시되지만
                                 // 실인수가 형식인수에 모두 대응되지 못하여 남는 형식인수의 경우에만 디폴트 초기값 적용
/* ==================================================================================================== */

function test2(str1, str2, str3 = str1 + str2) { // 디폴트 초기식의 디폴트값을 
    return str1 + str2 + str3;                   // 앞서 지정된 파라미터 값을 이용해 지정 가능
}
...
function concateString(...str) { // Rest parameter(가변인수): 전달한 실인수들을 배열 형태로 받아 참조를 반환
    let strCombine = '';
...
console.log(concateString('I ', 'am ', 'a good boy'));
...
function printName(name) {
    function print() {           // 로컬 이너 함수: 함수(외부 함수) 내부에 종속적인 지역 함수의 정의가 가능하며
        console.log(str + name); // 지역변수의 특성이 그대로 적용되어 외부에서는 접근 불가 
    }                            // 따라서, 로컬 이너 함수의 호출 호이스팅 범위는 주 함수 내로 제한됨
