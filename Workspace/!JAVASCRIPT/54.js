...
test1(5);             // 두 번째 인수값은 초기화되지 않아 undefined값이 할당되고 전달된 첫 번째 인수와의
                      // + 연산으로 인해 NaN값 반환 -> number + undefined = NaN
test1(7, 5, 4);       // 세 번째('4') 이후의 인수값은 참조를 잃어버려 직접 참조 불가
...
test5(test4);          // 함수명을 직접 참조로 전달 가능하며, 이를 통해 다른 함수 내에서도 전달한
                       // 함수의 참조를 통해 직접 호출 가능 -> 콜백 함수(Callback function)
...
// test6();         // 항상 호출시점이 기준이 되므로 호출시점 이전에 전역변수가 먼저 선언되어 있어야 함
...
function div(a, b) {
    if (isNaN(a) || isNaN(b)) {  // isNaN함수는 숫자형태의 문자열도 숫자로 인정
        console.log('입력값이 숫자가 아님');
    } else {
        console.log(a + " / " + b + " = " + (a / b));
    }
}
div("9", "4"); // 자바스크립트에서는 문자열 구분자("")로 묶어도 자료의 형태가 숫자 형태이면 숫자로 인식 가능
div("a", 4);   // "a" -> 
